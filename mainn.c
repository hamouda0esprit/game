...
int orientation2=0,xm2=100,ym2,jump2=1,dir2=2,velocity2,stop2=1,verif2,grav2=SCREEN_H-20,hit2=0,stopl=1,stopr=1;;
Personn p2;
bullett b2;
p2.cor.x=100;
p2.cor.y=grav2;
...
stop
*/
   loadanimationcaractere(NUM_FRAMESstopleft,framescarac,"stopr/stopr%d.png",SCREEN_W,SCREEN_H);
loadanimationcaractere(NUM_FRAMESstopright,frameslcarac,"stopl/stopl%d.png",SCREEN_W,SCREEN_H);
loadanimationcaractere(NUM_FRAMESright,framesrightcarac,"right/right%d.png",SCREEN_W,SCREEN_H);
loadanimationcaractere(NUM_FRAMESjump,framesjumpcarac,"jump/jump%d.png",SCREEN_W,SCREEN_H);
loadanimationcaractere(NUM_FRAMESleft,framesleftcarac,"left/left%d.png",SCREEN_W,SCREEN_H);
loadanimationcaractere(NUM_FRAMESjump,framessscarac,"stopp/stopaa%d.png",SCREEN_W,SCREEN_H);
		 ...
		 //---------------------------------------------COLLISION ENEMY/BULLET  TO RIGHT-------------------------------
					...
					vie_counter2++;
      				...
      				//-WHILE NOT COLLIDED-
     					 ...
     					 if (!(p.cor.x + p.cor.w/1.5 >= b2.pos.x + b2.pos.w && p.cor.x <= b2.pos.x && p.cor.y <= b2.pos.y && p.cor.y + p.cor.h >= b2.pos.y + b2.pos.h)&& hit2 == 1 && hitready2==1){
      				affbullet(screen,&b2);
      				xp2=1;}	
       				//-WHILE  COLLIDED-
      				 else{
