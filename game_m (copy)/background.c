...
void afficherBack(background bg, SDL_Surface * screen, int level){
	SDL_BlitSurface(bg.M[level-1] ,&bg.R2, screen, &bg.R);
}
...
void run_game(int playercount, int right, int left, int up, int *state,SDL_Rect *dest2,int* test,background* bg, player* P, SDL_Rect *rect, SDL_Surface* screen, int SCREEN_WIDTH, int SCREEN_HEIGHT, int *g_e_a, int WIDTH, int *anim_frame, int *anim_frame_time, Uint32 move_interval, Uint32 last_move_time, int *game_ended, int *trigger, int *done, SDL_Rect *dest, SDL_Rect *b1, SDL_Rect *b2, int *limit, int *level, int *movex, int t, int det_green, int det_red, int det_blue, int det_black,int *e1_stage, int *enigme1, int *enigme2){
			...
			printf("\n player pos x : %d", rect->x);
			printf("\n player pos y : %d", rect->y);
			//printf("\n limit : %d",*limit);
			//printf("\n enigme2 = %d", *enigme2);
			...
			*anim_frame_time+=1;
			...
			if(!(*game_ended)){
					...
					if (rect->x>1385){
						if (playercount == 1){
							...
							if (keys[SDLK_DOWN] && bg->R.y == 0){
								...
								dest->y-=SCREEN_HEIGHT*.85;
								rect->y=rect->y-SCREEN_HEIGHT/2;
								bg->R2.h = (SCREEN_HEIGHT*2);
							}
						}else if(playercount == 2){
							if ((keys[SDLK_z] || up) && bg->R.y < SCREEN_HEIGHT-SCREEN_HEIGHT/15){
								bg->R.y=SCREEN_HEIGHT-SCREEN_HEIGHT/15;
								b1->y+=SCREEN_HEIGHT*.85;
								b2->y+=SCREEN_HEIGHT*.85;
								dest->y+=SCREEN_HEIGHT*.85;
								rect->y=rect->y+SCREEN_HEIGHT/2;
							...
							if (keys[SDLK_s] && bg->R.y == SCREEN_HEIGHT-SCREEN_HEIGHT/15){
								bg->R.y=SCREEN_HEIGHT/15;
								b1->y-=SCREEN_HEIGHT*.85;
								b2->y-=SCREEN_HEIGHT*.85;
								dest->y-=SCREEN_HEIGHT*.85;
								rect->y=rect->y-SCREEN_HEIGHT/2;
				...
				if(*level == 3){
					if (playercount == 1){
						...
						if (det_black!=0 && bg->R.y < 0 && (keys[SDLK_z] || up)){
							bg->R.y=0;
							b1->y+=SCREEN_HEIGHT*.85;
							b2->y+=SCREEN_HEIGHT*.85;
							dest->y+=SCREEN_HEIGHT*.85;
						...
						if (det_black!=0 && keys[SDLK_s] && bg->R.y == 0){
							bg->R.y=-SCREEN_HEIGHT*.85;
							b1->y-=SCREEN_HEIGHT*.85;
							b2->y-=SCREEN_HEIGHT*.85;
							dest->y-=SCREEN_HEIGHT*.85;
			...
			if(*level==1 || *level==3){
				...
				if (bg->R.x <= -SCREEN_WIDTH*0.5275-SCREEN_WIDTH/3){
					bg->R.x = -SCREEN_WIDTH*0.5275-SCREEN_WIDTH/3;
					*limit = 1;
				}
				if ((bg->R.x > -SCREEN_WIDTH*0.51-SCREEN_WIDTH/3)&&(bg->R.x < 0)){
			       *limit = 0;
				}
			}
			if(*level==2){
			...
			*trigger = 0;
			...
			if(*limit){
				if(*level==1 || *level==3){
					if(bg->R.x >=(-SCREEN_WIDTH*0.5275)/2){
